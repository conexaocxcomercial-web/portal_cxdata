...
============================================
Versão 6.5: Ajustes Finos de Alinhamento (Header, Login e Password Toggle)
"""
...
class UIComponents:
    ...
    def input_field(label: str, password: bool = False, placeholder: str = '', icon: Optional[str] = None):
        with ui.column().classes('w-full gap-2 items-start'): # items-start garante alinhamento
            ...
            with ui.row().classes('w-full items-center relative'):
                ...
                input_field = ui.input(placeholder=placeholder, password=password).classes('w-full').props('outlined borderless').style(f'background: {DS.SURFACE}; border: 1.5px solid {DS.BORDER}; border-radius: {DS.RADIUS_MD}; padding-left: {"40px" if icon else "16px"};')
                
                # FIX: Adicionado o botão de "olhinho" nativo do Quasar/NiceGUI
                if password:
                    input_field.props('password-toggle-button')
    ...
    def sidebar_item(label: str, icon: str, active: bool = False, on_click=None):
        ...
        shadow = DS.SHADOW_SM if active else 'none'
        border = f'1px solid {DS.BORDER}' if active else '1px solid transparent'
        
        item = ui.row().classes('w-full items-center gap-3 px-3 py-2 cursor-pointer').style(f'''
            background: {bg_color};
    ...
    with ui.column().classes('w-full h-screen items-center justify-center').style(f'background: linear-gradient(135deg, {DS.SURFACE_50} 0%, {DS.PRIMARY_LIGHT} 100%);'):
        ...
        with ui.column().classes('w-full max-w-md px-8 relative z-10').style('gap: 32px;'):
            ...
            with ui.column().classes('w-full gap-6').style(f'background: {DS.SURFACE}; border: 1px solid {DS.BORDER_LIGHT}; border-radius: {DS.RADIUS_XL}; padding: 40px; box-shadow: {DS.SHADOW_MD};'):
                ui.label('Acesse seu workspace').classes('text-lg font-semibold').style(f'color: {DS.TEXT_PRIMARY};')
                
                email = UIComponents.input_field('Email', icon='mail')
                # FIX: Input de senha com o toggle nativo ativado pela função input_field
                senha = UIComponents.input_field('Senha', password=True, icon='lock')
                ...
                def try_login():
                    ...
                    if user:
                        ...
                        erro_label.text = 'Credenciais inválidas.'
                        erro_label.classes(remove='hidden')
                
                UIComponents.primary_button('Entrar', on_click=try_login, full_width=True, icon='arrow_forward')
                    ...
                    with ui.grid(columns='repeat(auto-fill, minmax(320px, 1fr))').classes('w-full').style('gap: 24px;'):
                        for idx, dash in enumerate(dashboards):
                            card = ui.column().classes('cursor-pointer group').style(f'''
                                background: {DS.SURFACE}; border: 1px solid {DS.BORDER}; border-radius: {DS.RADIUS_LG}; 
                                overflow: hidden; transition: all {DS.TRANSITION_FAST}; box-shadow: {DS.SHADOW_SM};
...
def page_dashboard(dash_id: int):
    ...
    with ui.column().classes('w-full h-screen').style(f'background: {DS.SURFACE_50}; margin: 0; padding: 0; overflow: hidden;'):
        ...
        # --- HEADER DO WORKSPACE (CORRIGIDO: POSICIONAMENTO E ALINHAMENTO) ---
        # Container do Topo
        with ui.column().classes('w-full').style(f'background: {DS.SURFACE}; border-bottom: 1px solid {DS.BORDER}; z-index: 50;'):
            
            # Linha Superior (Branding e Perfil)
            with ui.row().classes('w-full items-center justify-between').style(f'padding: 12px 24px; height: 64px;'):
                
                # ESQUERDA: Botão Voltar e Branding CX Data
                with ui.row().classes('items-center gap-3'):
                    # Botão voltar minimalista
                    ui.button(icon='arrow_back', on_click=lambda: ui.navigate.to('/')).props('flat round dense').style(f'color: {DS.TEXT_SECONDARY};')
                    
                    # Separador vertical sutil
                    ui.separator().classes('vertical').style(f'height: 24px; background: {DS.BORDER}; margin: 0 4px;')
                    
                    # Logo CX Data (Icon + Texto)
                    with ui.row().classes('items-center gap-2'):
                        with ui.column().classes('items-center justify-center').style(f'width: 28px; height: 28px; background: {DS.PRIMARY}; border-radius: 6px;'):
                            ui.icon('analytics', size='16px', color='white')
                        ui.label('CX Data').classes('text-base font-bold').style(f'color: {DS.TEXT_PRIMARY}; letter-spacing: -0.01em;')

                # DIREITA: Perfil do Usuário (Alinhado Corretamente)
                with ui.row().classes('items-center gap-3'):
                    # Texto (Nome e Email) alinhado à direita (items-end) para colar no avatar
                    with ui.column().classes('items-end justify-center gap-0'):
                        ui.label(cliente.nome).classes('text-sm font-bold').style(f'color: {DS.TEXT_PRIMARY}; line-height: 1.2;')
                        ui.label(user.email).classes('text-xs').style(f'color: {DS.TEXT_TERTIARY}; line-height: 1.2;')
                    
                    # Avatar
                    iniciais = ''.join([palavra[0].upper() for palavra in cliente.nome.split()[:2]])
                    with ui.column().classes('items-center justify-center').style(f'width: 36px; height: 36px; background: {DS.SURFACE_100}; border: 1px solid {DS.BORDER}; border-radius: 50%; color: {DS.PRIMARY}; font-size: 12px; font-weight: 700;'):
                        ui.label(iniciais)

            # Linha Inferior (Breadcrumbs / "Detalhes")
            # Agora fica DENTRO do header, logo abaixo, parecendo uma barra de ferramentas
            with ui.row().classes('w-full items-center gap-2').style(f'padding: 8px 24px; background: {DS.SURFACE_50}; border-top: 1px solid {DS.BORDER_LIGHT};'):
                ui.label('Workspaces').classes('text-xs font-medium cursor-pointer').style(f'color: {DS.TEXT_SECONDARY};').on('click', lambda: ui.navigate.to('/'))
                ui.icon('chevron_right', size='14px').style(f'color: {DS.TEXT_DISABLED};')
                ui.label(dash.nome).classes('text-xs font-bold').style(f'color: {DS.PRIMARY};')

        # --- ÁREA DO DASHBOARD (OCUPA O RESTO DA TELA) ---
