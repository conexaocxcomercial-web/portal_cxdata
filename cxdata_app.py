...
class UIComponents:
    ...
    def primary_button(text: str, on_click=None, full_width: bool = False, icon: Optional[str] = None):
        ...
        button.on('mousedown', lambda: button.style(f'''
            transform: translateY(0) scale(0.98);
        '''))
        
        button.on('mouseup', lambda: button.style(f'''
    ...
    def status_badge(text: str, status_type: str = 'success'):
        ...
        '''):
            ...
            ui.label(text).classes('text-xs font-semibold').style(f'''
                color: {color["text"]};
                letter-spacing: 0.02em;
            ''')
            ...
            '''):
                ...
                with ui.column().classes('gap-2 mb-2'):
                    ui.label('Welcome back').classes('text-xl font-semibold').style(f'''
                        ...
                        letter-spacing: -0.02em;
                    ''')
                    ui.label('Sign in to your workspace').classes('text-sm').style(f'''
                        color: {DS.TEXT_SECONDARY};
                    ''')
...
if __name__ in {'__main__', '__mp_main__'}:
    ...
    ui.run(
        ...
        host='0.0.0.0',
        port=port,
        storage_secret='cx_enterprise_2024_secure',
        reload=False
    )
